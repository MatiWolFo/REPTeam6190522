...
/*CREATE TABLE itAsignaturaUsuario(
id_it_asignatura_usuario int PRIMARY KEY NOT NULL AUTO_INCREMENT,
id_usuario int,
id_asignatura int
);*/
...
/*INTERTABLE DE RELACIONES usuario CON asignatura*/
/*REALIZAR LAS PREGUNTAS PARA VER SI SE PUEDE INTEGRAR CON MODULOS, CONTENIDOS, ETC, O SI ES REDUNDANTE, mati.
ALTER TABLE itAsignaturaUsuario ADD CONSTRAINT FK_usuario2ID FOREIGN KEY (id_usuario) REFERENCES usuarios(id_usuario);
ALTER TABLE itAsignaturaUsuario ADD CONSTRAINT FK_asignatura2ID FOREIGN KEY (id_asignatura) REFERENCES asignaturas(id_asignatura);*/

<<<<<<<< HEAD:CODIGOSPAULINO/SQL/tablesProyectoG6.sql
/*COMENTARIOS.
========
/*COMENTARIOS
>>>>>>>> updateMainDrop:_old/tablesProyectoG6.sql
...
PREGUNTAR SOBRE RELACION ENTRE USUARIO Y ASIGNATURAS, ES MUCHO A MUCHO? 
    CONTROL DE CAMBIOS: agregada intertable de usuario-asignatura, mati.
    CONTROL DE CAMBIOS: se saca, G6.
